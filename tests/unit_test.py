...
def test_generate_static_random_input() -> None:
    generate_config = InputGeneratorConfig(
    ...
    us = random_static_input(N=20, nu=2, config=generate_config)
...
def test_get_callable_from_method_name() -> None:
    method_name = 'np.sin'
    x = 0.5
    y = np.sin(x)
    sin = get_callable_from_method_string(method_name)

    assert isinstance(sin, Callable)
    assert y == sin(x)

    none = get_callable_from_method_string(None)

    assert none is None


def test_get_callable_from_input_config() -> None:
    x = 0.5
    y = np.sin(x)
    config = InputGeneratorConfig(
        type='np.sin', u_max=1, u_min=-1, interval_min=2, interval_max=10
    )
    sin = get_callable_from_input_config(config)

    assert isinstance(sin, Callable)
    assert y == sin(x)

    none = get_callable_from_input_config(None)
