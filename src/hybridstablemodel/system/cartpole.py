...
class CartPole(DynamicSystem):
    ...
    def state_dynamics(self, x, u):
        ...
        x_dot[3] = (
            ...
            * (
                (
                    -u
                    - self.m_p * self.length * x4**2 * np.sin(x3)
                    + self.mu_c * np.sign(x2)
                )
                / (self.m_c + self.m_p)
            )
            ...
            self.length
            * (4 / 3 - (self.m_p * np.cos(x3) ** 2) / (self.m_c + self.m_p))
        )
        x_dot[0] = x2
        x_dot[1] = (
            u
            + self.m_p * self.length * x4**2 * np.sin(x3)
            - self.m_p * self.length * x_dot[3] * np.cos(x3)
            - self.mu_c * np.sign(x2)
        ) / (self.m_c + self.m_p)
        x_dot[2] = x4
        return x_dot
    ...
    def get_linearization(
        ...
        A = np.array(
            [
                [
                    diff(x1_dot, x1).evalf(subs=eval_dict),
                    diff(x1_dot, x2).evalf(subs=eval_dict),
                    diff(x1_dot, x3).evalf(subs=eval_dict),
                    diff(x1_dot, x4).evalf(subs=eval_dict),
                ],
                [
                    diff(x2_dot, x1).evalf(subs=eval_dict),
                    diff(x2_dot, x2).evalf(subs=eval_dict),
                    diff(x2_dot, x3).evalf(subs=eval_dict),
                    diff(x2_dot, x4).evalf(subs=eval_dict),
                ],
                [
                    diff(x3_dot, x1).evalf(subs=eval_dict),
                    diff(x3_dot, x2).evalf(subs=eval_dict),
                    diff(x3_dot, x3).evalf(subs=eval_dict),
                    diff(x3_dot, x4).evalf(subs=eval_dict),
                ],
                [
                    diff(x4_dot, x1).evalf(subs=eval_dict),
                    diff(x4_dot, x2).evalf(subs=eval_dict),
                    diff(x4_dot, x3).evalf(subs=eval_dict),
                    diff(x4_dot, x4).evalf(subs=eval_dict),
                ],
            ]
        )
