...
def plot_states(result: SimulationData) -> None:
    ...
    for element, ax in zip(range(nx), axs[:, 0]):
        ax.plot(result.t, np.array([x[element] for x in result.xs]))
        ax.set_title(f'$x_{element+1}$')
        ax.grid()


def plot_inputs(result: SimulationData) -> None:
    ...
    for element, ax in zip(range(nu), axs[:, 0]):
        ax.plot(result.t, np.array([u[element] for u in result.us]))
        ax.set_title(f'$u_{element+1}$')
        ax.grid()


def plot_outputs(result: SimulationData) -> None:
    ...
    for element, ax in zip(range(ny), axs[:, 0]):
        ax.plot(result.t, np.array([y[element] for y in result.ys]))
        ax.set_title(f'$y_{element+1}$')
        ax.grid()


def plot_comparison(
    results: List[SimulationData], type: Literal['xs', 'us', 'ys']
) -> None:
